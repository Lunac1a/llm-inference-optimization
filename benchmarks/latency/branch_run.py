...
def launch(settings, job, out):
    ...
    env['PYTHONPATH'] = str(ROOT / 'candidate-source' if candidate else ROOT / 'baseline-source')
    env['INFERENCE_MIXED_BLOCK_SOURCE_BRANCH'] = '1' if candidate else '0'
...
def worker(job):
    ...
    settings = Settings(profile='chunked-hybrid', model=MODEL,
                        backend_url='http://127.0.0.1:8034', log_dir=out / 'engine')
    ...
    if job.startswith('B'): assert 'block_source_branch=True' in logs
...
def execute(job, limit):
    ...
    command = [sys.executable, str(Path(__file__).with_name('branch_micro.py'))] if job == 'micro' else [sys.executable, __file__, '--job', job]
    env = dict(os.environ)
...
def main():
    ...
    paths += list((ROOT / 'candidate-source').rglob('*.py'))
    paths += list(Path(__file__).parent.glob('*.py'))
    paths += [WORKSPACE / 'docs/mixed-branch-protocol.md', WORKSPACE / 'docs/mixed-split-protocol.md']
