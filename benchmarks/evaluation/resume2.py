...
def used():
    return original.used() + sum(r['seconds'] for r in rows(ROOT / 'resume-1/ledger.jsonl')) + (sum(r['seconds'] for r in rows(RESUME / 'ledger.jsonl'))
...
def main():
    verify_original(); verify_previous()
    ...
    paths = [Path(__file__), Path(__file__).with_name('report_resumed2.py'),
             WORKSPACE / 'benchmarks/evaluation/keep_awake2.py', WORKSPACE / 'docs/final-benchmark-resume2-protocol.md']
