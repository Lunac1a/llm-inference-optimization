"""Audit all responses before producing public, version-specific summaries."""
...
def main():
    ...
    assert len(ledger) == 19 and all(r['reason'] == 'completed' and r['code'] == 0 for r in ledger)
    ...
    for length in LENGTHS:
        ...
        for arm in ARMS:
            ...
            for block in [0, 1]:
                out = ROOT / 'runs' / f'{length}-{block}-{arm}'
    ...
    summary = {'version': '0.1.3', 'quality_responses': 2340, 'unique_quality_inputs': 780,
        ...
        'fixed_output_timing': timing, 'gpu_lifecycle_seconds': sum(r['seconds'] for r in ledger),
    ...
    lines += ['', '## 固定输出延迟', '', '每块3次测量；两块反向组别顺序。以下为首个非空SSE内容时间，不是设备端精确token延迟。', '',
