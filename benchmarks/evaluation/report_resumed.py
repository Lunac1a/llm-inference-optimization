...
import numpy as np
from config import *
from resume import RESUME, verify_original, verify_resume
...
def main():
    ...
    write(export / 'provenance.json', {'frozen_hashes': read(ROOT / 'frozen.json'), 'environment': read(ROOT / 'environment.json'),
        ...
        'resume_frozen_hashes': read(RESUME / 'frozen.json'), 'original_ledger': initial, 'resume_ledger': continued,
    ...
    lines += ['', '## 范围与完整性', '',
        ...
        '- 用户授权续跑保留原1040响应、24延迟记录及BF16热身超时失败；10个缺失数据块在resume-1独立保存，未重跑已完成数据。',
