...
import numpy as np
from config import *
from resume2 import RESUME, verify_original, verify_resume, verify_previous
...
def main():
    verify_original(); verify_previous(); verify_resume()
    ...
    assert len(continued) == 10 and all(r['reason'] == 'completed' and r['code'] == 0 for r in initial[:-1] + continued)
    failed_resume = rows(ROOT / 'resume-1/ledger.jsonl')
    assert len(failed_resume) == 1 and failed_resume[0]['reason'] == 'child_failed'
    ledger = initial + failed_resume + continued
    ...
    for length in LENGTHS:
        ...
        for arm in ARMS:
            ...
            for block in [0, 1]:
                ...
                out = (RESUME if (RESUME / 'runs' / job / 'summary.json').exists() else ROOT) / 'runs' / job
                quality = rows(out / 'quality.jsonl'); latency = rows(out / 'latency.jsonl')
                assert len(quality) == 130 and len(latency) == 3 and len(rows(out / 'warmup.jsonl')) == 2
                block_expected = {key: r for key, r in expected.items() if block * 10 <= r['sample'] < (block + 1) * 10}
                validate_records(quality, block_expected)
                for record in quality + latency + rows(out / 'warmup.jsonl'):
                ...
                summary = read(out / 'summary.json'); capacities.append(summary['capacity_tokens'])
    ...
    summary = {'version': '0.1.3', 'quality_responses': 2340, 'unique_quality_inputs': 780,
        ...
        'timing_caveats': {'4096': 'pre-resume Modern Standby exposure', '8192': 'spans interrupted and resumed segments', '16384': 'entirely resumed segment; inspect host events'},
    ...
    write(export / 'provenance.json', {'frozen_hashes': read(ROOT / 'frozen.json'), 'environment': read(ROOT / 'environment.json'),
        ...
        'resume_frozen_hashes': read(RESUME / 'frozen.json'), 'original_ledger': initial, 'failed_resume_ledger': failed_resume, 'resume_ledger': continued,
    ...
    lines += ['', '## 固定输出延迟（4K/8K受待机或跨段影响）', '', '每块3次测量；两块反向组别顺序。以下为首个非空SSE内容时间，不是设备端精确token延迟。', '',
    ...
    lines += ['', '## 范围与完整性', '',
        ...
        '- 用户授权续跑保留原1040响应、24延迟记录及BF16热身超时失败；10个缺失数据块在resume-2独立保存；前两次失败均保留，未重跑已完成数据。',
        '- 4K包含Windows现代待机风险，8K跨越中断前后，时间只作描述，不能用于强加速结论。16K全部在临时防自动休眠的续跑段完成，另核查宿主电源事件。',
