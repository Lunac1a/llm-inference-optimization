...
# Validated Prefill tile; Decode retains the upstream small-query geometry.
PREFILL_BLOCK_M = 64
...
def kernel_unified_attention(
    ...
    current_key_ptr,
    current_value_ptr,
    current_stride_0: tl.int64,
    current_stride_1: tl.int64,
    value_stride_0: tl.int64,
    value_stride_1: tl.int64,
    sink_ptr,
    block_tables_ptr,
    seq_lens_ptr,
    alibi_slopes_ptr,
    ...
    BLOCK_SOURCE_BRANCH: tl.constexpr = False,
):
    ...
    for j in range(loop_lo, loop_hi):
        ...
        history_mask = tile_mask & (seq_offset < context_len)
        current_mask = tile_mask & (seq_offset >= context_len)

        if BLOCK_SOURCE_BRANCH:
            if (j + 1) * TILE_SIZE <= context_len:
                # Uniform CTA branch: complete historical tile.
                history_mask = tile_mask
                ...
                if USE_TD:
                    ...
                    K_load = tl.load(
                        ...
                        mask=dim_mask[:, None] & history_mask[None, :],
                        other=0.0,
                    )
                    # V : (TILE_SIZE, HEAD_SIZE)
                    V_load = tl.load(
                ...
                if USE_PER_TOKEN_HEAD_SCALES:
                    ...
                    k_token_head_scales = tl.load(
                        k_scale_cache_ptr + scale_idx, mask=history_mask, other=1.0
                    )
                    v_scale_idx = (
                    ...
                    v_token_head_scales = tl.load(
                        v_scale_cache_ptr + v_scale_idx, mask=history_mask, other=1.0
                    )

            elif j * TILE_SIZE >= context_len:
                # Complete current tile: no page-table or scale-cache reads.
                current_mask = tile_mask
                current_index = cur_batch_in_all_start_index + seq_offset - context_len
                current_k = tl.load(
                    current_key_ptr + current_index[None, :] * current_stride_0
                    + kv_head_idx * current_stride_1 + offs_d[:, None],
                    mask=dim_mask[:, None] & current_mask[None, :], other=0.0)
                current_v = tl.load(
                    current_value_ptr + current_index[:, None] * value_stride_0
                    + kv_head_idx * value_stride_1 + offs_d[None, :],
                    mask=dim_mask[None, :] & current_mask[:, None], other=0.0)
                K, V = current_k, current_v
                k_token_head_scales = tl.full((TILE_SIZE,), 1.0, tl.float32)
                v_token_head_scales = tl.full((TILE_SIZE,), 1.0, tl.float32)
            else:
                # At most one tile intersects the historical/current boundary.
                physical_block_idx = tl.load(
                    block_tables_ptr + block_table_offset + seq_offset // BLOCK_SIZE,
                    mask=tile_mask, other=0,
                ).to(tl.int64)
                ...
                current_v = tl.load(
                    ...
                    + kv_head_idx * value_stride_1 + offs_d[None, :],
                    mask=dim_mask[None, :] & current_mask[:, None], other=0.0)
                K = tl.where(current_mask[None, :], current_k, K)
...
def unified_attention(
    ...
    current_key=None,
    current_value=None,
    block_source_branch: bool = False,
):
    assert causal, "Only causal attention is supported"
    assert not use_td, "Dual-source extension supports pointer loads only"
    assert kv_quant_mode == KVQuantMode.FP8_PER_TOKEN_HEAD
    assert current_key is not None and current_value is not None
    assert q.dtype == current_key.dtype == current_value.dtype == torch.bfloat16
    assert current_key.shape == current_value.shape
    assert current_value.stride(-1) == 1
    assert current_key.stride(-1) == 1
    ...
    BLOCK_M = (
        max(PREFILL_BLOCK_M, triton.next_power_of_2(num_queries_per_kv)) if max_seqlen_q > 1 else max(16, triton.next_power_of_2(num_queries_per_kv))
    )
    ...
    kernel_unified_attention[grid](
        ...
        key_cache_ptr=k,
        value_cache_ptr=v,
        current_key_ptr=current_key,
        current_value_ptr=current_value,
        current_stride_0=current_key.stride(0),
        current_stride_1=current_key.stride(1),
        value_stride_0=current_value.stride(0),
        value_stride_1=current_value.stride(1),
        sink_ptr=sinks,
