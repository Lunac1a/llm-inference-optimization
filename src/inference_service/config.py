...
Profile = Literal["document", "adaptive", "hybrid", "cpu-kv"]
PROFILES = ("document", "adaptive", "hybrid", "cpu-kv")
...
class Settings:
    ...
    def __post_init__(self):
        ...
        if not 1 <= self.port <= 65535 or self.request_timeout <= 0:
            raise ValueError("Invalid port or request timeout")
        if self.batch_invariant and self.profile != "adaptive":
            raise ValueError("Batch invariance is exposed only for the separately validated adaptive profile")
    ...
    def from_env(cls):
        return cls(model=os.getenv("INFERENCE_MODEL", cls.model),
                   ...
                   batch_invariant=os.getenv("INFERENCE_BATCH_INVARIANT", "0") == "1",
