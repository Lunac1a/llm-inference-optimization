...
"""Owned native KV offload adapter; no changes to default launcher."""
import json
import os
...
if __name__ == '__main__':
    ...
    os.environ['VLLM_USE_SIMPLE_KV_OFFLOAD'] = '0'
    if mode == 'offload':
    ...
    Path(os.environ['KV_OFFLOAD_LAUNCH']).write_text(json.dumps({'argv': sys.argv,
        'environment': {k: os.environ.get(k) for k in ('VLLM_PLUGINS', 'VLLM_USE_SIMPLE_KV_OFFLOAD',
