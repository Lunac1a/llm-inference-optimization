...
cleanup() {
    cp .tmp/stage2/vllm-server.log artifacts/stage2/server-recovery-final.log || true
    bash scripts/stop-vllm-baseline.sh > artifacts/stage2/cleanup-recovery.log 2>&1 || true
    nvidia-smi >> artifacts/stage2/cleanup-recovery.log
    ss -ltnp >> artifacts/stage2/cleanup-recovery.log
}
trap cleanup EXIT
bash scripts/start-vllm-baseline.sh > artifacts/stage2/start-recovery.log 2>&1
...
timeout 180 "$py" scripts/stage2-run.py recovery-warmup 512 128 4 --count 128
"$py" scripts/stage2-run.py recovery-anchor1 512 128 4
"$py" scripts/stage2-run.py recovery-anchor2 512 128 4
"$py" - <<'PY'
import json
from pathlib import Path
p=Path('artifacts/stage2')
a,b=[json.loads((p/f'recovery-anchor{i}/summary.json').read_text())['output_throughput_tok_s'] for i in (1,2)]
d=abs(b-a)/a*100
